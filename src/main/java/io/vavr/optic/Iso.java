...
public final class Iso<S, A> extends PIso<S, S, A, A> {
    ...
    public <S1, A1> Iso<Tuple2<S, S1>, Tuple2<A, A1>> product(final Iso<S1, A1> other) {
        return new Iso<>(pIso.product(other.pIso));
    }

    @Override
    public <C> Iso<Tuple2<S, C>, Tuple2<A, C>> first() {
        return new Iso<>(pIso.first());
    }

    @Override
    public <C> Iso<Tuple2<C, S>, Tuple2<C, A>> second() {
